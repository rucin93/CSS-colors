...
function search() {
    ...
    for(w='',i=e=2;e+=hash(e,res.charCodeAt(i/2));i++) {
        ...
        if(e.toString(16)[y] === undefined) {
            console.log(i/2|0, res.charCodeAt(i/2) , 'undefined', 'e:', e, 'w:', e.toString(16)[y]||'0', 'm:', m[i-2])  // Debugging output
            errors++
        }
        
        if ((m[i-2] && e.toString(16)[y]) != m[i-2] ) {
            console.log(i/2|0, res.charCodeAt(i/2), 'Not matching', 'e:', e, 'w:', e.toString(16)[y]||'0', 'm:', m[i-2])  // Debugging output
            errors++
        }
    }

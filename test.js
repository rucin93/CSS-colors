  ...
  class Encoder {
    constructor(initialState, targetHex, targetHexPairs, cacheSize) {
      ...
      for (let char = 1; char < 0xfff; char++) {
        if (isValidChar(char)) {
            this.possibleChars.push(String.fromCodePoint(char));
        }
      }
    }
...
//   const targetHex = "0f8fffae"; // Target hex string
  ...
  const encoder = new Encoder(initialState, targetHex, targetHexPairs, 100); // Cache size of 1000
  ...
  if (result) {
    ...
    fs.writeFile(new Date().getTime() + '.txt', `for(w='f',i=e=2;e+=e**.2*\`-${result.history.join("")}\`.codePointAt(i++/2);)w+=e.toString(16)[${INDEX}]`, (err) => {
        if (err) {
            console.log(err)
        }
    })

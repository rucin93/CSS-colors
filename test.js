  ...
  class Encoder {
    ...
    generateNextStates(state) {      
      ...
      for (const char of possibleChars) {
        ...
        const currentHash = createHash(state.start, char.codePointAt(), currentPair.length);
    ...
    encode() {
      ...
      while (!completed) {
        ...
        console.log((++counter).toString().padStart(3, ' '), ' of ', this.targetHexPairs.length, 'New Cache:', this.newCache.size)
...
//   const targetHex = "0f8fffae"; // Target hex string
  ...
  const targetHexPairs = targetHex.split``.reverse().join``.match(/.{1,2}/g); // Create pairs from the hex string
  console.log(targetHexPairs)

  const initialState = new State({
    start: 2, currentPairIndex: 0, byteCount: 0, history: []
  }); 
  const encoder = new Encoder(initialState, targetHex, targetHexPairs, 16*10); // Cache size of 1000
  ...
  if (result) {
    ...
    fs.writeFile("out/" + 'js' + '.txt', `for(w=i=e=2;e+=e**.1*\`-${result.history.join("")}\`.codePointAt(i++/2);)w=e.toString(16)[${INDEX}]+w`, (err) => {
        if (err) {
            console.log(err)
        }
    })
